...
//            this.employeesFromDepartmentV2();
//            this.getEmployeeWithProjectV2();
//            this.increaseSalariesV2();
//            this.increaseSalariesV3();
//            this.employeesMaximumSalariesV2();
    ...
    //5.2. Employees from Department
    private void employeesFromDepartmentV2() {
        String query =
                "SELECT e FROM Employee e " +
                        "WHERE e.department.name = 'Research and Development' " +
                        "ORDER BY e.salary, e.id";

        List<Employee> employees = this.em.createQuery(query, Employee.class).getResultList();
    ...
    private void addingANewAddressAndUpdatingEmployee() throws IOException {
        ...
        CriteriaBuilder builder = this.em.getCriteriaBuilder();
        try {
            address = this.em.createQuery("SELECT a FROM Address a WHERE a.text = :addressName", Address.class)
    ...
    private void addressesWithEmployeeCount() {
        List<Address> addresses = this.em
                .createQuery("SELECT a FROM Address AS a " +
                        "ORDER BY size(a.employees) DESC, a.town.id", Address.class)
                .setMaxResults(10)
                .getResultList();
    ...
    //8.2. Get Employee with Project
    private void getEmployeeWithProjectV2() throws IOException {
        int employeeId = Integer.parseInt(this.br.readLine());
    ...
    //10.2 Increase Salaries
    private void increaseSalariesV2() {
        List<Integer> departmentIds = new ArrayList<>(Arrays.asList(1, 2, 4, 11));
        this.em.createQuery(
                "UPDATE Employee e " +
                        "SET e.salary = e.salary * 1.12 " +
    ...
    //10.3 Increase Salaries
    private void increaseSalariesV3() {
        List<Integer> departmentIds = new ArrayList<>(Arrays.asList(1, 2, 4, 11));
    ...
    //13.2. Employees Maximum Salaries
    private void employeesMaximumSalariesV2() {
        List<Employee> employees = this.em.createQuery(
                "SELECT e " +
                        "FROM Employee e " +
        ...
        employees.forEach(e ->
                System.out.printf("%s - %.2f%n", e.getDepartment().getName(),
                        e.getDepartment().getEmployees().stream()
                                .mapToDouble(emp ->
                                        Double.parseDouble(emp.getSalary().toString()))
                                .max().orElse(0)));
    }
}
