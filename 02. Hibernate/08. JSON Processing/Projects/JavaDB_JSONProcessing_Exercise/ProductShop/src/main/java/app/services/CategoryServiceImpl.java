...
public class CategoryServiceImpl implements CategoryService {
    ...
    //Always save on the owning side. Which means that save the many to many from the Product side, not the categories side like here!!
    @Override
    @Transactional
    public String seedCategories(CategoryDto[] categoryDtos) {
    ...
    @Override
    public List<CategoryCountDto> categoriesByProductCount() {
        List<Category> categories = this.categoryRepository.categoriesByProductsCount();
        return categories.stream().map(c -> this.mapper.map(c, CategoryCountDto.class))
